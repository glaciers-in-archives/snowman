...
func DiscoverViews(layouts []string) ([]View, error) {
	...
	for _, viewConf := range vConfigs.Views {
		...
		if viewConf.Unsafe {
			TextTemplateA, err = text_template.New("").Funcs(function.GetQueryFuncs()).Funcs(function.GetStringFuncs()).Funcs(function.GetMathFuncs()).Funcs(function.GetUtilsFuncs()).Funcs(function.GetIncludeFuncs()).Funcs(function.GetJSONFuncs()).Funcs(function.GetRemoteFuncs()).ParseFiles(templates...)
		} else {
			HTMLTemplateA, err = html_template.New("").Funcs(function.GetQueryFuncs()).Funcs(function.GetStringFuncs()).Funcs(function.GetMathFuncs()).Funcs(function.GetUtilsFuncs()).Funcs(function.GetIncludeFuncs()).Funcs(function.GetJSONFuncs()).Funcs(function.GetRemoteFuncs()).ParseFiles(templates...)
		}
